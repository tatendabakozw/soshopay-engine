...
import { Expenses, PAYGLoanDetails } from '@/lib/types';

type Props = {
  setLoanDetails: React.Dispatch<React.SetStateAction<PAYGLoanDetails>>;
  handleSubmit: (e: React.FormEvent) => void;
  loanDetails: PAYGLoanDetails;
  loading: boolean;
}

function PaygLoan({setLoanDetails, handleSubmit, loanDetails, loading}: Props) {
  const handleInputChange = (field: keyof PAYGLoanDetails) => (value: any) => {
    setLoanDetails((prev) => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-white md:p-8 p-4 rounded-xl border border-zinc-300/50">
      ...
      <PrimaryInput
        label="Product Usage Hours"
        placeholder="Enter daily product usage hours"
        value={loanDetails.productUsageHours}
        setValue={handleInputChange('productUsageHours')}
        type="number"
      />
      <div className="flex flex-col">
        <label htmlFor="productVerified" className="text-zinc-950 font-semibold pl-1 pb-2">Product Verified</label>
        <select
          id="productVerified"
          value={loanDetails.productVerified ? 'Yes' : 'No'}
          onChange={(e) => handleInputChange('productVerified')(e.target.value === 'Yes')}
          className="border p-3 text-base rounded-xl border-zinc-300/50 bg-white text-zinc-600"
        >
          <option value="Yes">Yes</option>
          <option value="No">No</option>
      ...
      <PrimaryInput
        ...
        value={loanDetails.subscriptionCost}
        setValue={handleInputChange('subscriptionCost')}
        type="number"
      />
      <button 
        type="submit" 
